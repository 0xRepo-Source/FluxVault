...
func main() {
	...
	if *version {
		fmt.Println("FluxVault-admin version: 0.1.0-lite")
		return
	}
...
func printUsage() {
	fmt.Printf(`FluxVault-admin - Token management tool

USAGE:
  FluxVault-admin <command> [options]
...
EXAMPLES:
  FluxVault-admin create -user alice -permissions * -days 365
  FluxVault-admin create -user bob -permissions upload,download -days 90
  FluxVault-admin list
  FluxVault-admin revoke tok_abc123
...
func revokeCommand() {
	if len(os.Args) < 3 {
		fmt.Println("Usage: FluxVault-admin revoke <token_id> [-file <tokens.json>]")
		os.Exit(1)
	}
